...
export const useDatosEmployeePorManagerId = (idManager) => {
    const [datosEmployeePorManagerId,setDatosEmployeePorManagerId]=useState([]);
  
    const url='http://52.23.156.12:8085/employee/'+idManager+'/datosDeEmpleadosDeUnManagerPorId2';
    ...
    const fetchData = useCallback(async () => {
      try {
        ...
        if (response.status === 200) {
          setDatosEmployeePorManagerId(response.data);
        }
      } catch (error) {
  ...
  return [datosEmployeePorManagerId];
}
...
export const useDatosEmployeePorManagerId = (idManager) => {
    ...
    const url='http://localhost:8085/employee/${idManager}/datosDeEmpleadosDeUnManagerPorId2';
