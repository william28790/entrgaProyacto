...
export const useCantidadDeEmpleadosPorBanda = () => {
    const [cantidadDeEmpleadosPorBanda,setCantidadDeEmpleadosPorBanda]=useState([]);

    const url='http://52.23.156.12:8087/contract/cantidadDeEmpleadosPorBanda';
    ...
    const fetchData = useCallback(async () => {
      try {
        ...
        if (response.status === 200) {
          setCantidadDeEmpleadosPorBanda(response.data);
        }
      } catch (error) {
  ...
  return [cantidadDeEmpleadosPorBanda];
}
